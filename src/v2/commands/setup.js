...
function importTasklist(taskList, taskConfig, usherFilePath, parser) {
	...
	const tasks = _.endsWith(taskConfig.from, ".yml")
		? loadAndParseYmlFile(
				path.join(usherFilePath, taskConfig.from),
				"tasks",
				parser
		  )
...
module.exports = (config, Logger, usherFilePath, parser = require("./parse")) =>
	Promise.try(() => {
		_.mapValues(validators, (validator) => validator(config));
		const modulesToInstall = interpolateModulesToInstall(config);
		return Promise.all(_.map(modulesToInstall, installModule)).then(() => {
			const reducedTasks = _.reduce(
				config.include,
				(acc, includeConfig) =>
					importTasklist(acc, includeConfig, usherFilePath, parser),
				{}
			);
			const reducedArgs = _.reduce(
				config.include,
				(acc, includeConfig) =>
