...
/* Mock functions declared underneath tests */
jest.mock('./dependencies/npm-install-include', () => () => {});
jest.mock('./dependencies/require-include', () => mockRequireInclude); 
...
describe('src/v3/lib/resolve-dependencies', function() {
  ...
  test('dependencies are resolved correctly for shell task with remote dependencies', function() {
    const taskName = 'foo';
    const foo = { actions: [ { do: 'bar.baz' } ] };
    const fooUsherfile = { includes: { bar: { from: 'shell-relative-dependencies' } }, tasks: { foo } };
    const baz = { actions: [ { do: 'shell', command: 'echo qux' } ] };
  ...
  test('dependencies are resolved correctly for loop task with remote dependencies', function() {
    const taskName = 'foo';
    const foo = { actions: [ { do: 'for', each: 'bar', in: 'baz', exec: 'qux.quux' } ] };
    const fooUsherfile = { includes: { qux: { from: 'loop-relative-dependencies.yml' } }, tasks: { foo } };
    const quux = { actions: [ { do: 'shell', command: 'echo corge'} ] };
  ...
  test('nested dependencies are resolved correctly for shell task with remote dependencies', function() {
    const taskName = 'foo';
    const foo = { actions: [ { do: 'bar.baz' } ] };
    const fooUsherfile = { includes: { bar: { from: 'nested-shell-relative-dependencies.yml' } }, tasks: { foo } };
    const baz = { actions: [ { do: 'qux' } ] };
  ...
  test('nested dependencies are resolved correctly for loop task with remote dependencies', function() {
    const taskName = 'foo';
    const foo = { actions: [ { do: 'for', each: 'bar', in: 'baz', exec: 'qux.quux' } ] };
    const fooUsherfile = { includes: { qux: { from: 'nested-loop-relative-dependencies.yml' } }, tasks: { foo } };
    const quux = { actions: [ { do: 'corge' } ] };
...
function mockRequireInclude(moduleName) {
  switch (moduleName) {
    case 'loop-relative-dependencies':
      return { version: '3', tasks: { quux: { actions: [ { do: 'shell', command: 'echo corge' } ] } } };
    case 'nested-loop-relative-dependencies':
      return { version: '3', tasks: { quux: { actions: [ { do: 'corge' } ] }, corge: { actions: [ { do: 'shell', command: 'echo uier' } ] } } };
    case 'nested-shell-relative-dependencies':
      return { version: '3', tasks: { baz: { actions: [ { do: 'qux' } ] }, qux: { actions: [ { do: 'shell', command: 'echo quux' } ] } } };
    case 'shell-relative-dependencies':
      return { version: '3', tasks: { baz: { actions: [ { do: 'shell', command: 'echo qux' } ] } } };
  }
};

function mockParseFile(filepath) {
